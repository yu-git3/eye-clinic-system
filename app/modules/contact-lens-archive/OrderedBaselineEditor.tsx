...
export function CheckCard({group,readOnly,onRevise,onReport,headerAction}:{group?:Check;readOnly:boolean;onRevise:(g:string,i:string,e:"od"|"os",v:string)=>void;onReport:(g:Check)=>void;headerAction?:React.ReactNode}){if(!group)return null;const meta=group.source==="医技检查"&&group.reporterName?`报告人：${group.reporterName}　报告时间：${group.reportTime}`:`${group.source==="护士采集"?"采集人":"录入人"}：${group.enteredBy}　${group.source==="护士采集"?"采集时间":"录入时间"}：${group.enteredAt}`;return <div className="cl-ordered-card"><div className="sr-check-header-wrap"><CheckHeader title={group.group} meta={meta} group={group} onReport={onReport}/>{headerAction}</div>{composite.has(group.group)?<>{(["od","os"] as const).map(eye=><div className="cl-composite-eye" key={eye}><strong>{eye.toUpperCase()}</strong>{group.rows.map(row=><label key={row.item}><span>{row.item}</span><input disabled={readOnly} value={row[eye]} onChange={e=>onRevise(group.group,row.item,eye,e.target.value)}/><i>{row.unit}</i></label>)}</div>)}{group.group==="角膜地形图"&&<div className="cl-composite-summary">{(["od","os"] as const).map(eye=><span key={eye}>{eye.toUpperCase()}：Ks {group.rows.find(r=>r.item==="Ks曲率")?.[eye]}D@{group.rows.find(r=>r.item==="Ks轴位")?.[eye]}°；MinK {group.rows.find(r=>r.item==="MinK曲率")?.[eye]}D@{group.rows.find(r=>r.item==="MinK轴位")?.[eye]}°</span>)}</div>}</>:<table><thead><tr><th>检查指标</th><th>OD</th><th>OS</th><th>单位</th></tr></thead><tbody>{group.rows.map(row=><tr key={row.item}><td>{row.item}</td><td><input disabled={readOnly} value={row.od} onChange={e=>onRevise(group.group,row.item,"od",e.target.value)}/></td><td><input disabled={readOnly} value={row.os} onChange={e=>onRevise(group.group,row.item,"os",e.target.value)}/></td><td>{row.unit||"—"}</td></tr>)}</tbody></table>}</div>}
