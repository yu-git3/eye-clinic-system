...
import { changeTreatmentMethod, createArchiveSeed, createArchiveSeeds, referenceCheckReport, reopenArchive, reviseCheckValue, terminateArchive, validateArchiveDraft, validateBaseline, type Baseline, type ContactLensArchive, type ReportCandidate, type TreatmentMethod } from "./archive-store";
...
function RecordTab({ archive, readOnly, eyeHealthExam, onEyeHealthChange, onRevise, onReport, onCreate, onLoad, onBaseline, onChange, onTerminate, onReopen }: { archive: ContactLensArchive | null; readOnly: boolean; eyeHealthExam: ExamInstance; onEyeHealthChange: (exam: ExamInstance) => void; onRevise: (group: string, item: string, eye: "od" | "os", value: string) => void; onReport: (group: ContactLensArchive["checks"][number]) => void; onCreate: () => void; onLoad: () => void; onBaseline: () => void; onChange: () => void; onTerminate: () => void; onReopen: () => void }) {
    return <SpecialtyRecordModule archive={archive} checks={archive?.checks ?? createArchiveSeed().checks} readOnly={readOnly} eyeHealthExam={eyeHealthExam} onEyeHealthChange={onEyeHealthChange} onRevise={onRevise} onReport={onReport} onCreateArchive={onCreate} onLoadArchives={onLoad} onEditBasic={onBaseline} onChangeMethod={onChange} onTerminate={onTerminate} onReopen={onReopen}/>;
